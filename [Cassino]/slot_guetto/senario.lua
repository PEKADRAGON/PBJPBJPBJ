addEvent("guetto-asksenario", true)
addEvent("guetto-gamefinished", true)
...
function slot.asksenario ( count, show, bet, farm, scatters )
    ...
    if bet then
        ...
        setElementData(client, "validateMoney", true)
        triggerEvent("kumar:server", client, "takemoney", amount)
    end
end
addEventHandler("guetto-asksenario", root, slot.asksenario)
...
addEventHandler("guetto-gamefinished", root, slot.finishgame)
...
function slot.createsenario ( player, spinamount, show, bet, farm, scatters )
    ...
    if devmode then
        ...
        outputChatBox(formatNumber(totalwin), root, 255, 255, 255)
    else
        triggerClientEvent(player, "guetto-sendsenario", root, slot.game, show, farm)
    end
end
...
addCommandHandler("guettodevmode", function(plr, cmd, st)
    ortalama = 0
    if st == "test" and devmode then
        local macsayisi = 100
        local bet = 10000
        outputChatBox(formatNumber(bet * 100).."faça uma aposta "..formatNumber(macsayisi).." resultado da partida", root, 255, 255, 255)
        for i=1, macsayisi do
            slot.createsenario(nil, 11, nil, bet)    
        end
        outputChatBox(formatNumber(macsayisi).."Ganhos médios de até: ".. formatNumber(ortalama / macsayisi), root, 0, 255, 0)
    elseif st == "state" then
        devmode = not (devmode or false)
        outputChatBox("devmode: "..(devmode and "Abrir" or "fechado"), plr, 255, 0, 0)
    end
end)