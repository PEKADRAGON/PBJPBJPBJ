...
addEventHandler("dede:panel", root, panelStatement)
...
function askSenario ( count, show, bet, farm, scatters )
    triggerServerEvent("guetto-asksenario", root, count, show, bet, farm, scatters)
end
...
addEvent("guetto-sendsenario", true)
function getSenarioFromServer ( senario, show, farm )
    ...
    setTimer(function()
        slot.game, startgame, spincount = senario, getTickCount(), "Carregando jogo.."
        exploded = nil
        explodestreak = 0
        skipscreen = nil
        freespinadded = false
...
addEventHandler("guetto-sendsenario", root, getSenarioFromServer)

--
function checkMoney ( amount )
    if (exports["slot_core"]:getSlotMoney()) >= amount then
        return true
    end
    return false
end
