...
function drawWinScreen ( amount, type, clicktype, start )
    ...
    dxDrawOutlineText("PARABÉNS!", x, y * 1.5, x + w, nil, tocolor(255, 215, 36, alpha), 1, ui.fonts["top_3"], "center", "center", true, false, tocolor(0, 0, 0, alpha))
    dxDrawOutlineText("VOCÊ GANHOU", x, y * 2, x + w, nil, tocolor(193, 159, 114, alpha), 1, ui.fonts["top_3"], "center", "center", true, false, tocolor(0, 0, 0, alpha))
    if true then
       ...
       -- dxDrawImage(x, y, _w, _h, ui.screen["wintype"], 0, 0, 0, tocolor(255, 255, 255, alpha), true)
        dxDrawOutlineText(amount, x, y, x + _w, y + _h, tocolor(172, 205, 132, alpha), 1, ui.fonts["top_3"], "center", "center", true, false, tocolor(0, 0, 0, alpha))
    end
    --10 #C19F72GRANDE-ASSALTO
    dxDrawOutlineText(type, x, y * 3.35, x + w, nil, tocolor(255, 255, 255, alpha), 1, ui.fonts["top_3"], "center", "center", true, true)
    dxDrawBorderedText(1, tocolor(0, 0, 0, alpha), 'PRESSIONE EM QUALQUER LUGAR PARA CONTINUAR', x, y * 3.8, x + w, y * 3.8, tocolor(255, 255, 255, alpha), 1, ui.fonts["top_2.5"], 'center', 'center', false, false, true)
...
function drawFreeSpinScreen ( start )
    ...
    dxDrawImage(x, y, w, h, ui.screen["winscreen"], 0, 0, 0, tocolor(255, 255, 255, alpha), true)
    dxDrawBorderedText(0.9, tocolor(0, 0, 0, alpha), '+5\nGRANDE-ASSALTO', x, y, x + w, y + h, tocolor(254, 254, 67, alpha), size, ui.fonts["top_2.75"], 'center', 'center', false, false, true)
end
...
function drawSpinWinScreen ( old, new, multiplier, start )
    ...
    local y = y + (10 * psy)
    dxDrawBorderedText(0.85, tocolor(217, 14, 27, alpha), (formatNumber(math.floor(new)).. (multiplier and " x "..multiplier.." " or " ") .."R$"), x, y, x + w, y + h, tocolor(254, 240, 1, alpha), 0.85, ui.fonts["top_2.75"], "center", "center", false, false, true)
end
...
function drawClose ( x, y, w, h )
    local mouse = isMouseInPosition(x, y, w, h)
    if mouse then
...
function drawBackground ( )
    ...
    if not spinwinscreen or (spinwinscreen and getTickCount() >= (spinwinscreen[4] + 2500)) then
                    ...
                    setTimer(function()
                        ...
                        if slot.game[gameindex + 1] then
                            ...
                            if gameindex ~= 1 then
                                ...
                                showWinScreen(formatNumber((totalwin or 0)).." R$", totalspincount.." #C19F72GRANDE-ASSALTO", "gamefinished")
                            else
                                onFarmFinish ( )
                            end
                        end
        ...
        dxDrawImage(cx + (320 * psx), cy + (20 * psy)-10, 953 * psx, 693 * psy, ui.screen["title"], 0, 0, 0, tocolor(255, 255, 255, alpha))
    else
    ...
    if juststay and not skipscreen and not winscreen then
        ...
        local checkMouse = isMouseInPosition((x - (w / 2)) + (15 * psx), (y - (h / 2)) + (15 * psy), w - (30 * psx), h - (30 * psy))
        ...
        dxDrawBorderedText(1, tocolor(146, 146, 146), ' GRANDE ASSALTO ', x+2, y + (25 * psy) - 2, x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_1"], 'center', 'top')
        dxDrawBorderedText(1, tocolor(146, 146, 146), '10 RODADAS ', x, y + (45 * psy) +1, x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_1"], 'center', 'top')
        dxDrawBorderedText(1, tocolor(146, 146, 146), "R$ "..formatNumber(slot.bets[slot.betindex] * (100)).."", x, y + (65 * psy)+3, x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_2"], 'center', 'top')
    end
    --bottom menu
    ...
    dxDrawOutlineText("CRÉDITO ", cx + (280 * psx), cy + (25 * psy), nil, nil, tocolor(246, 175, 49), 1, ui.fonts["bottom_1"], "left", "top")
    dxDrawOutlineText( " R$ "..formatNumber(getPlayerMoney(localPlayer))..".00", cx + (360 * psx), cy + (25 * psy), nil, nil, tocolor(255, 255, 255), 1, ui.fonts["bottom_1"], "left", "top")
    dxDrawOutlineText("APOSTA ", cx + (280 * psx), cy + (55 * psy), nil, nil, tocolor(246, 175, 49), 1, ui.fonts["bottom_1"], "left", "top")
    dxDrawOutlineText( " R$ "..formatNumber(slot.bets[slot.betindex])..".00", cx + (360 * psx), cy + (55 * psy), nil, nil, tocolor(255, 255, 255), 1, ui.fonts["bottom_1"], "left", "top")

    if not juststay then
        if tonumber(spincount) then
            dxDrawOutlineText("RODADAS GRATUITAS RESTANTES  "..((spincount + 1) - gameindex), cx + (615 * psx), cy + (60 * psy), nil, nil, tocolor(255, 255, 255), 1, ui.fonts["bottom_1"], "left", "top")
        end
        dxDrawOutlineText("PRÊMIOS #ffffffR$ "..formatNumber((totalwin or 0)).."", cx + (775 * psx), cy - (5 * psy), nil, nil, tocolor(246, 175, 49), 1, ui.fonts["bottom_2"], "center", "top", false, true)
    else
        dxDrawOutlineText("DEFINA SUA APOSTA E JOGUE!", cx + (635 * psx), cy + (25 * psy), nil, nil, tocolor(255, 255, 255), 1, ui.fonts["bottom_1"], "left", "top")
    end
    --farmbutton
    if true then
        ...
        if not (spinindex ~= 1 and multipliers[gameindex]) then
            ...
            if slot.game[gameindex + 1] then
                ...
                if gameindex ~= 1 then
                    removeAllSounds()
                    setSoundVolume(giveSound("background-music.oga", true), 0.2)
                    showWinScreen(formatNumber(totalwin).." R$", totalspincount.." #C19F72GRANDE-ASSALTO", "gamefinished")
                else
                    onFarmFinish ( )
                end
            end
    ...
    if not freespinadded and candy == "candy" then 
        if gameindex == 1 then
            if #slot.game > 1 and not freespinfromfarm then
                setTimer(showWinScreen, 500, 1, 10, "#C19F72GRANDE-ASSALTO", "setfreespin")
            else
...
function showWinScreen ( amount, type, clicktype )
    if type == "#C19F72GRANDE-ASSALTO" then
        giveSound("free-spin-start.oga")
    else
        giveSound("free-spin-end-bg.oga")
        triggerServerEvent("guetto-gamefinished", root)
    end
    juststay = true
...
function interactionColor ( x, y, w, h, color1, color2, elm )
    ...
    return isMouseInPosition(x, y, w, h) and color2 or color1
end
