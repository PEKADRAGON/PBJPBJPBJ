...
function drawWinScreen ( amount, type, clicktype, start )
    ...
    dxDrawOutlineText("VOCÊ GANHOU", x, y * 2, x + w, nil, tocolor(0, 207, 190, alpha), 1, ui.fonts["top_3"], "center", "center", true, false, tocolor(0, 0, 0, alpha))
    if true then
...
function drawBackground ( )
                    ...
                    setTimer(function()
                        ...
                        if slot.game[gameindex + 1] then
                            ...
                            if gameindex ~= 1 then
                                ...
                                showWinScreen(formatNumber((totalwin or 0)).." R$", totalspincount.." #00cfbeFREESPINLER", "gamefinished")
                            else
                                onFarmFinish ( )
                            end
                        end
    ...
    if juststay and not skipscreen and not winscreen then
        ...
        dxDrawBorderedText(1.25, tocolor(240, 52, 155), 'MODO TURBO', x, y + (25 * psy), x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_1"], 'center', 'top')
        dxDrawBorderedText(1.25, tocolor(240, 52, 155), '10 RODADAS', x, y + (45 * psy), x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_1"], 'center', 'top')
        dxDrawBorderedText(1, tocolor(240, 52, 155), "R$ "..formatNumber(slot.bets[slot.betindex] * (100)).."", x, y + (65 * psy), x + w, y + h, tocolor(255, 255, 255), ts, ui.fonts["top_2"], 'center', 'top')
    end
    --bottom menu
        ...
        if not (spinindex ~= 1 and multipliers[gameindex]) then
            ...
            if slot.game[gameindex + 1] then
                ...
                if gameindex ~= 1 then
                    removeAllSounds()
                    setSoundVolume(giveSound("background-music.oga", true), 0.2)
                    showWinScreen(formatNumber(totalwin).." R$", totalspincount.." #00cfbeFREESPINLER", "gamefinished")
                else
                    onFarmFinish ( )
                end
            end
