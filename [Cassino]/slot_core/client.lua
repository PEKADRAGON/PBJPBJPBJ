...
local font = dxCreateFont("fontbold.ttf", 9 * psy)
local f2ont = dxCreateFont("fontbold.ttf", 17 * psy)

games = {
    {dxCreateTexture("dede.png"), "dede:panel", true, "dede"},
    {dxCreateTexture("sweet.png"), "sweet:panel", true, "sweet"},
    --{dxCreateTexture("sweet.png"), "arcade:panel", true, "arcade"},
}
...
function renderUI ( )
    if true then
        ...
        dxDrawText("JOGOS RECOMENDADO PARA MAIORES DE 18 ANOS", cx, cy, cx + csx, cy + (20 * psy), tocolor(225, 225, 225, 255), 1, font, "center", "bottom")
    end
    ...
    for i, v in ipairs(games) do
        ...
        local checkMouse = isMouseInPosition(x, y, icsw, icsh)
...
function onClickLib ( elm, tick, ... )
    if elm == "close" then
        ...
        if state then
            ...
            outputChatBox("Este sistema não está ativo no momento.", 255, 0, 0)
        end
    end
end
...
function isSlotAvailable ( )
    ...
    if slot then
        if (not getElementData(slot, "slot-player") or not isElement(getElementData(slot, "slot-player"))) then
            setElementData(slot, "slot-player", lp)
            current_slot = slot
            return true
        end
        return false
...
addEventHandler("onClientRender", root, function()
    ...
    if slot then
        ...
        if x and y then
            ...
            dxDrawText(slotplaying and "Este caça-níquel é usado atualmente por outra pessoa" or "Pressione a tecla 'e' para jogar caça-níqueis.", x, y, nil, nil, slotplaying and tocolor(168, 79, 79, 225) or tocolor(225, 225, 225, 225), 1, f2ont, "center", "center")
        end
    end
end)
