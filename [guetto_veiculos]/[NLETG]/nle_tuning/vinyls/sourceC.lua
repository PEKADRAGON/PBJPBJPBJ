	...
	function()
		if state then
			...
			for i = 1,countX*countY do
				...
				local imgPath = ":nle_vinyls/stickers/" .. stickers[category].dir .. "/" .. i+scroll .. ".dds"
			...
			if not categorySelect and stickerID <= stickers[category].stickerCount then
				dxDrawText("Preço do adesivo selecionado: #74b347" .. format(stickers[category].price) .. "GP",pos[1],pos[2]+size[2]+5,nil,nil,tocolor(200,200,200,200*alpha),1,fonts["default_bold_10"],"left","top",false,false,false,true)
			end

			dxDrawNavButton(pos[1]+size[1]-80-55,pos[2]+size[2]+5,55,25,"ENTER","default_bold_10",alpha)
			dxDrawText("Seleção",pos[1]+size[1],pos[2]+size[2]+35/2,nil,nil,tocolor(200,200,200,200*alpha),1,fonts["default_10"],"right","center")

			dxDrawNavButton(pos[1]+size[1]-220-90,pos[2]+size[2]+5,90,25,"BACKSPACE","default_bold_10",alpha)
			dxDrawText("Recuar",pos[1]+size[1]-140,pos[2]+size[2]+35/2,nil,nil,tocolor(200,200,200,200*alpha),1,fonts["default_10"],"right","center")
			...
			dxDrawText("Navegação",pos[1]+size[1],pos[2]+size[2]+35+35/2,nil,nil,tocolor(200,200,200,200*alpha),1,fonts["default_10"],"right","center")
		end
	end
)
	...
	function(k,p)
		if state and p then
			...
			elseif k == "enter" then
				...
				if not categorySelect and stickerID <= stickers[category].stickerCount then
					exports.nle_vinyls:addNewSticker(getPedOccupiedVehicle(localPlayer),stickers[category].dir,stickerID)
					toggleStickerBrowser(false)
					local x,y,rot = exports.nle_vinyls:getPaintjobDefaults(getPedOccupiedVehicle(localPlayer))
