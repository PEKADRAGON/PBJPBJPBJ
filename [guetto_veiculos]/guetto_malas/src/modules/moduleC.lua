...
function removeEventHandler(eventName, attachedTo, theFunction)
    if isEventHandlerAdded(eventName, attachedTo, theFunction) then 
...
function destroyElements ( )
    ...
    for index, value in pairs ( svgRectangles ) do 
        if value and isElement(value) then 
            destroyElement(value)
        end
    end
...
slidebar = {
    ...
    draw = function ( )
        ...
        for i, v in pairs(slidebar.data) do 
            ...
            dxDrawImage(v.pos[1] + (v.progress - 9), v.pos[2] + v.pos[4] / 2 - 18 / 2, 18, 18, "assets/images/icon-slide.png", 0, 0, 0, tocolor(255, 255, 255, 255), true)
