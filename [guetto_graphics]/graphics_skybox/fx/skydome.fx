...
//-- PixelShader
//-----------------------------------------------------------------------------
PSOut PixelShaderFunction(PSInput PS)
{
    PSOut output;
    ...
    float4 color = float4( texel.rgb, texel.a * gColorMul.a );
	
    if ( fFogEnable ) color.rgb = AddObjectSpaceDepth( color.rgb, PS.PositionVS.z );
    color.a *= gColorAlpha;
