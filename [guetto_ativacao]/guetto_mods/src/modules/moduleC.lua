...
local rectangles = {}
local strokes = {}
function dxDrawRectangleRounded(x, y, w, h, radius, color, stroke, post)
    radius = radius * scale
    ...
    if stroke then
        if not strokes[index] then
            ...
            dxDrawImageBlend(x, y, w, h, strokes[index], stroke[1], 0, 0, 0, post)
        end
    end
end
...
function saveCacheToXML(active)
    local xml_save_log_File = xmlLoadFile('cache.xml')
    if not xml_save_log_File then
        xml_save_log_File = xmlCreateFile('cache.xml', 'login')
    end
    local usernameNode = xmlFindChild(xml_save_log_File, 'active', 0)
    if not usernameNode then
        usernameNode = xmlCreateChild(xml_save_log_File, 'active')
    end
    xmlNodeSetValue(usernameNode, tostring(active))
    xmlSaveFile(xml_save_log_File)
    xmlUnloadFile(xml_save_log_File)
end

function loadCacheFromXML()
    local xml_save_log_File = xmlLoadFile('cache.xml')
    if not xml_save_log_File then
        xml_save_log_File = xmlCreateFile('cache.xml', 'login')
    end
    ...
    if usernameNode then
        return xmlNodeGetValue(usernameNode) == 'true'
    else
        return false
    end
    xmlUnloadFile(xml_save_log_File)
