...
addEventHandler("setPedAnimation", root, function(block,anim,time,loop,updatePosition,interruptable, freezeLastFrame)
	...
	if block then
		...
		setElementData(source, "animation", {block, anim})
	else
		setPedAnimation(source, nil)
		removeElementData(source, "animation")
	end
end)
...
addCommandHandler("resetmesa", function(plr,cmd,tn)
	--if getElementData(localPlayer, "ID") ~= 2 then return end
	local tn=tonumber(tn)
	if not tn then
