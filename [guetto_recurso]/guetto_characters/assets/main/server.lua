...
local function onPlayerOpenCharacters(player)
    ...
    local clothes = exports["guetto_custom"]:getPedClothes(account_name)
...
registerEvent("guetto.select.character", resourceRoot, function ( index, dados )
    ...
    if (index == 1) then 
        clothes = exports["guetto_custom"]:getPedClothes(dados.accountName)
    else
        clothes = exports["guetto_custom"]:getPedClothes(dados.login)
    end
